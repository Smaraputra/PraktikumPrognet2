                                    ...
                                    @if ($statusgambar == 1)
                                        <div class="col-md-8">
                                            <img class="d-block w-100"
                                                src="/uploads/product_images/{{ $image->image_name }}" alt="First slide">
                                        </div>
                                    @else
                                        <div class="col-md-8">
                ...
                <div class="col-md-4">
                        ...
                        <ul class="unstyle">
                            ...
                            @if ($products->discount->count())
                                ...
                                @php
                                    ...
                                    $statusDiskon = 0;
                                @endphp
                                @foreach ($products->discount as $key => $diskon)
                                ...
                                @foreach ($products->discount as $key => $diskon)
                                    @if (strtotime($diskon->start) <= $current_date && strtotime($diskon->end) >= $current_date)
                                        ...
                                        <span class="price">
                                            <span
                                                class="edd_price">Rp.{{ number_format($products->price * ((100 - $diskon->percentage) / 100)) }}</span>
                                        </span>
                                        @php
                    ...
                    @if (!is_null(Auth::user()))
                        @if ($products->stock == 0)
                            ...
                            <br>
                            <button type="button" class="btn btn-warning btn-lg btn-block" disabled>Add To
                                Cart</button>
                        @else
            ...
            <div class="tab-content" id="myTabContent">
                ...
                @else
                    @foreach ($products->product_review as $item)
                        ...
                        <div class="row mb-5">
                            <div class="col-sm-2">
                                <img src="{{ asset('/uploads/avatars/' . $item->user->profile_image) }}"
                        ...
                        @if ($item->response->count())
                            @foreach ($item->response as $balasan)
                                <div class="row mb-5">
                                    ...
                                    <div class="col-sm-2">
                                        <img src="{{ asset('/uploads/avatars/' . $balasan->admin->profile_image) }}"
                                            style="width:100px;height:100px;object-fit:cover;"
                                            class="avatar rounded-circle z-depth-1-half">
                                    </div>
                                    <div class="col-sm-9">
                                        <h5 class="font-weight-bold"><span style="margin-right:5px;"
                                                class="badge badge-success">Admin</span>{{ $balasan->admin->name }}
                                        </h5>
